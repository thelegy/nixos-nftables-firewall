system: nixpkgs: lfinal: lprev:
with lfinal; {
  run-tests = tests: let
    pkgs = nixpkgs.legacyPackages.${system};
    testResults = runTests tests;
    showResults = result:
      readFile (pkgs.runCommand "${result.name}-diff" {} ''
  ...
  in
    ...
    then traceSeqN 10 (map showResults testResults) (throw "At least one tests did not match its expected output")
    else pkgs.emptyDirectory;

  machineTest = module: let
  ...
  in
    ...
    then traceSeqN 10 failedAssertions (throw "At lease one test did not hold all assertions")
