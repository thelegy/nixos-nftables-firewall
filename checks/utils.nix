system: nixpkgs: lfinal: _lprev: with lfinal; {
  run-tests =
    ...
    let
      ...
      showResults =
        result:
        readFile (
          pkgs.runCommand "${result.name}-diff" { } ''
    ...
    if length testResults > 0 then
      traceSeqN 10 (map showResults testResults) (
        throw "At least one tests did not match its expected output"
      )
    else
      pkgs.emptyDirectory;
  ...
  machineTest =
    ...
    in
    if length failedAssertions > 0 then
