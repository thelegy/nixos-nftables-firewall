...
in
{
            ...
            config =
              ...
              in
              {
                assertions = flatten [
                  {
                    assertion =
                      config.ignoreEmptyRule
                      || hasAllowedTCPPorts
                      || hasAllowedUDPPorts
                      || hasAllowedTCPPortRanges
                      || hasAllowedUDPPortRanges
                      || hasVerdict
                      || hasMasquerade
                      || hasExtraLines;
                    message = ''
  ...
  config =
    let
      toPortList =
        ports:
        assert length ports > 0;
        "{ ${concatStringsSep ", " (map toString ports)} }";

      toRuleName = rule: "rule-${rule.name}";
      toTraverseName =
        from: matchFromSubzones: to: matchToSubzones: ruleType:
        let
      ...
      lookupZones =
        zoneNames: if zoneNames == "all" then singleton allZone else map (x: zones.${x}) zoneNames;
      zoneInList =
        zone: zoneNames:
        if zone.name == "all" then zoneNames == "all" else isList zoneNames && elem zone.name zoneNames;
      ...
      childZones =
        parent:
        if parent.name == "all" then
          filter (x: x.name != "all" && !x.localZone && isNull x.parent) sortedZones
        else
          filter (x: x.parent == parent.name) sortedZones;
    in
    mkIf cfg.enable {
      assertions = flatten [
      ...
      networking.nftables.chains =
        ...
        in
        {
          ...
          forward.generated.rules = concatLists (
