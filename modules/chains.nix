    ...
    { config, ... }:
    {
      ...
      config =
        ...
        in
        {
          ...
          text = mkMerge [
            (mkIf isGoto (renderRule [
              "goto"
              r.goto
            ]))
            (mkIf r.isJump (renderRule [
              "jump"
              r.jump
            ]))
          ];
          ...
          processedRule =
            let
              ...
              textRule = simplifyRule (renderRule [
                r.onExpression
                r.text
              ]) r.chainDependencies "";
              ...
              inlinedRule = simplifyRule (renderRule [
                r.onExpression
                inlineableRule.processedRule.text
              ]) (inlineableRule.processedRule.deps or [ ]) (inlineableRule.processedRule.comment or comment);
              jumpRule =
                if chainRules.${r.jump} == [ ] then
                  { }
                else if inlinable then
                  inlinedRule
                else
                  textRule;
            in
            if r.isJump then jumpRule else textRule;
        };
    }
  );
